...
`.env`에 Supabase 프로젝트의 URL과 **publishable key**를 설정하세요. 설정하지 않으면 안전하게 데모 데이터로 동작합니다. 브라우저 코드에는 secret/service role 키를 절대 넣지 마세요.

## Supabase 설정

1. Supabase 프로젝트를 생성합니다.
2. `supabase/migrations`의 SQL을 Dashboard SQL Editor 또는 Supabase CLI로 적용합니다.
3. 회원가입 후 고용주 계정의 `profiles.role`은 신뢰된 서버/관리자 경로에서 `employer`로 변경합니다.
4. AI 에이전트는 서버 환경에서만 secret key를 사용해 `matches`를 생성하고, 클라이언트에는 publishable key만 제공합니다.

마이그레이션은 모든 공개 테이블에 RLS를 적용하며, 프로필의 역할 변경과 포인트 원장 생성은 일반 클라이언트에서 허용하지 않습니다. 실제 출금 전에는 서버 함수에서 현재 원장 잔액, 중복 요청, 계좌 소유자를 다시 검증해야 합니다.
